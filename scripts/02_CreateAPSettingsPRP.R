...
#LandTrendr Coordinates (i.e. EPSG 5070)
Coord.LandTrendr = CRS("+init=epsg:5070")
...
RunDtmDescribe <- function(DtmDirectory, OutputDirectory, OutputName, ALL=TRUE, WHICH=NULL, fpFUSION){
	...
	shell(
		paste0(fpFUSION, "DTMDescribe.exe ", DtmFileTxt, " ", OutputDirectory, OutputName, "_Summary.csv")
		)
}
...
#Outputs:
#	Text for section 8
writeSection8 <- function(PRODUCTHOME, PROCESSINGHOME, fpFUSION){
	#Run DtmDescribe on the density DTM
	RunDtmDescribe(DtmDirectory=paste0(PRODUCTHOME, "QAQC\\"), OutputDirectory=PROCESSINGHOME, OutputName="DTM_Density", ALL=FALSE, WHICH="QAQC_return_count", fpFUSION = fpFUSION)
	#Read in the CSV created with RunDtmDescribe()
...
writePRP <- function(PROJECT, LATITUDE, HOMEFOLDER, PROCESSINGHOME, DIRSCRIPTS,
				...
				nBlocksHigh, nBlocksWide, DtmDescribe, nCores, fpFUSION){
	...
	#Density data
	writeSection8(PRODUCTHOME = PRODUCTHOME, PROCESSINGHOME = PROCESSINGHOME, fpFUSION = fpFUSION)
...
createPRP <- function(studyArea, cellSize, nCores, DIR_BASE, DIRSCRIPTS, DIR_LIDAR, fpFUSION){
	...
	#DIR_LIDAR <- file.path("D:", "workflowTest", studyArea, "Points", "LAZ5070")
	...
	RunDtmDescribe(DtmDirectory = DTMSPEC, OutputDirectory = PROCESSINGHOME, OutputName = "DTM_Ground", fpFUSION = fpFUSION)
