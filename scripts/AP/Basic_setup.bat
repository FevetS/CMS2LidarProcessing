...
REM Added by Fekety 2015.18.12
PATH=%PATH%;C:\Fusion
...
REM DOINTENSITY controls whether intensity images are created
...
SET DOCANOPY=TRUE
SET DOGROUND=TRUE
SET DOINTENSITY=FALSE
SET DOTOPO=TRUE

SET DOFIRSTSTRATA=TRUE
...
REM Make sure the projection file is the correct format (all data on 1 line) or else the conversion from ASCII raster to either formats
...
REM should divide evenly into the basic cell size.
IF /I [%UNITS%]==[feet] (
	SET CELLSIZE=98.424
	SET HTCUTOFF=4.5
	SET COVERCUTOFF=4.5
...
REM ###########################################################
REM Set the identifier for ground surface models
...
IF "%BLOCKNAME%"=="" (
   SET PRODUCTHOME=%WORKINGDIRNAME%\Products_Blocks
   REM SET FINALPRODUCTHOME=%WORKINGDIRNAME%\Products_%AREA%_%AP_RUNDATE%
   SET FINALPRODUCTHOME=%WORKINGDIRNAME%
) ELSE (
   SET PRODUCTHOME=%WORKINGDIRNAME%\Products_Blocks\%BLOCKNAME%

   REM create the directory name for the merged layers
   REM SET FINALPRODUCTHOME=%WORKINGDIRNAME%\Products_%AREA%_%AP_RUNDATE%\FINAL_%AREA%_%AP_RUNDATE%
   SET FINALPRODUCTHOME=%WORKINGDIRNAME%
)
...
REM IF /I "%DOTOPO%"=="true" MKDIR "%PRODUCTHOME%\TopooMetrics_%FILEIDENTIFIER%
...
IF NOT EXIST "%FINALPRODUCTHOME%\Scripts" (
	...
	REM COPY "%WORKINGDIRNAME%\%RUNIDENTIFIER%_DeliveryTiles.*" "%FINALPRODUCTHOME%\Layout_shapefiles"
	REM COPY "%WORKINGDIRNAME%\%RUNIDENTIFIER%_ProcessingTiles.*" "%FINALPRODUCTHOME%\Layout_shapefiles"
	REM COPY "%WORKINGDIRNAME%\%RUNIDENTIFIER%_ProcessingBlocks.*" "%FINALPRODUCTHOME%\Layout_shapefiles"
	MOVE "%PROCESSINGHOME%\%RUNIDENTIFIER%_DeliveryTiles.*" "%FINALPRODUCTHOME%\Layout_shapefiles" /Y
	MOVE "%PROCESSINGHOME%\%RUNIDENTIFIER%_ProcessingTiles.*" "%FINALPRODUCTHOME%\Layout_shapefiles" /Y
	MOVE "%PROCESSINGHOME%\%RUNIDENTIFIER%_ProcessingBlocks.*" "%FINALPRODUCTHOME%\Layout_shapefiles" /Y
)
